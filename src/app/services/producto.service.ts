...
export class ProductoService {
    ...
    return this.http.get<IProducto[]>(`${this.apiUrl}/GetAllProductos`)
      .pipe(
        catchError(error => {
          ...
          return throwError(() => new Error('Error al obtener los productos. Por favor, inténtalo de nuevo.'));
        })
      );
  }
  ...
  getProductosPaged(page: number = 0, size: number = 10): Observable<any> {
    ...
    console.log(`Obteniendo productos paginados (página: ${page}, tamaño: ${size})`);
    ...
    return this.http.get<any>(`${this.apiUrl}/GetProductosByMarcaPaged`, { params })
      .pipe(
        catchError(error => {
          ...
          return throwError(() => new Error('Error al buscar productos por marca. Por favor, inténtalo de nuevo.'));
        })
      );
  }
    ...
    return this.http.get<any>(`${this.apiUrl}/GetProductosByTipoComidaPaged`, { params })
      .pipe(
        catchError(error => {
          ...
          return throwError(() => new Error('Error al buscar productos por tipo de comida. Por favor, inténtalo de nuevo.'));
        })
      );
  }
    ...
    return this.http.get<any>(`${this.apiUrl}/GetProductosByTituloPaged`, { params })
      .pipe(
        catchError(error => {
          ...
          return throwError(() => new Error('Error al buscar productos por título. Por favor, inténtalo de nuevo.'));
        })
      );
  }
    ...
    return this.http.get<IProducto>(`${this.apiUrl}/GetProductoById/${id}`)
      .pipe(
        catchError(error => {
          ...
          return throwError(() => new Error('Error al obtener el producto. Por favor, inténtalo de nuevo.'));
        })
      );
  }
  ...
  createProducto(producto: IProducto): Observable<IProducto> {
    ...
    return this.http.post<IProducto>(`${this.apiUrl}/CreateProducto`, producto, { headers })
      .pipe(
        catchError(error => {
          console.error('Error al crear producto:', error);
          return throwError(() => new Error('Error al crear el producto. Por favor, inténtalo de nuevo.'));
        })
      );
  }
  ...
  updateProducto(id: number, producto: IProducto): Observable<IProducto> {
    ...
    return this.http.put<IProducto>(`${this.apiUrl}/UpdateProducto/${id}`, producto, { headers })
      .pipe(
        catchError(error => {
          ...
          if (error.status === 0) {
            console.warn('Posible error CORS detectado. Intentando método alternativo...');
  ...
  private updateProductoWithFetch(id: number, producto: IProducto): Observable<IProducto> {
    return new Observable(observer => {
      ...
      .catch(error => {
        ...
        observer.error(new Error('Error al actualizar el producto usando método alternativo.'));
      });
    });
  }
  ...
  calcularNutrientes(productoId: number, gramos: number): Observable<any> {
    ...
    console.log(`Calculando nutrientes para producto ID: ${productoId}, cantidad: ${gramos}g`);
    return this.http.get<any>(`${this.apiUrl}/CalcularNutrientes`, { params })
      .pipe(
        catchError(error => {
          console.error('Error al calcular nutrientes:', error);
          return throwError(() => new Error('Error al calcular los nutrientes. Por favor, inténtalo de nuevo.'));
        })
      );
  }
  ///////////////////////////////////////////////////
